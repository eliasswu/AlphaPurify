...
class FactorAnalyzer():
    ...
    def trace_symbols(self, rebalence_period:int, symbol:list, dates:list=None, position="l", return_df=False, staticPlot:bool=False, return_fig:bool=False):
        date = datetime.datetime.strptime(date,'%Y-%m-%d %H:%M:%S')
        df = self.calc_stats_for_trace(rebalence_period,date)


    def calc_stats_for_period_t(self,args):
        period, _worker_df = args
        ###########################################贴标签分箱
        df = _worker_df.clone()
        ...
        if self.group_by:
            df_ind = df.with_columns([
                pl.col(self.symbol_col)
                .map_elements(lambda x: self.group_by.get(x, "Unlabels"), return_dtype=pl.Utf8)
                .alias("industry")
            ])
        ...
        ######################################################s
        cum_nv = df_result[f"cum_ret_net_s_{self.bins}"].to_numpy()
        mean_ret_s = df_result.select(pl.col(f"ret_net_s_{self.bins}").mean()).item()
        mean_loss = (df_result.filter(pl.col(f"ret_net_s_{self.bins}") < 0).select(pl.col(f"ret_net_s_{self.bins}").mean()).item())
        PL = mean_ret_s / mean_loss
        ann_ret = cum_nv[-1]** (365.25 / self.days) - 1
        vol = (df_result.select(pl.col(f"ret_net_s_{self.bins}").std(ddof=1)).item()) * np.sqrt(annual_factor)
        if np.isnan(vol):
            vol = (df_result.select(pl.col(f"ret_net_s_{self.bins}").std(ddof=0)).item()) * np.sqrt(annual_factor)
        ...
        win_rate = float(df_result.select((pl.col(f"ret_net_s_{self.bins}") > 0).mean()).item())
        pnl = float(cum_nv[-1] - 1) if cum_nv.size > 0 else np.nan
        downside_std = (df_result.filter(pl.col(f"ret_net_s_{self.bins}") < 0).select(pl.col(f"ret_net_s_{self.bins}").std(ddof=1)).item())
        if downside_std is not None and not np.isnan(downside_std):
            downside_vol = downside_std * np.sqrt(annual_factor)
            sortino = excess_ret / downside_vol if downside_vol != 0 else np.nan
        else:
