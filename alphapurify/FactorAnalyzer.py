...
class ResearchConfig:
    rebalance_periods: list[int | str] = ("W","M","Q")
    return_rolling_period: int = 20
    return_horizons: list[int | str] = (1,5,10)
...
class FactorAnalyzer():
    ...
    def calc_stats_for_period(self,args):
        ...
        df = df.filter(pl.col("quantile").is_in([1, self.mid_q, self.bins]))
        rebalance_df = rebalance_df.filter(pl.col("quantile").is_in([1, self.mid_q, self.bins])).sort(by=[self.trade_date_col, "quantile",self.symbol_col])
        ####################################buqi
        ...
        common = df_prev.join(
            df_next,
            on=[self.trade_date_col, "quantile", "g_prev", "g_next", "code"],
            how="inner"
        )
        ...
        df_agg_q1 = df_agg.filter(pl.col("quantile") == 1).rename({
                "buy": "buy_q1",
                "sell": "sell_q1",
                "turnover": "turnover_q1",
                "cost": "cost_q1"
        }).drop("quantile")
        
        df_agg_qm = df_agg.filter(pl.col("quantile") == self.mid_q).rename({
                "buy": "buy_qm",
                "sell": "sell_qm",
                "turnover": "turnover_qm",
                "cost": "cost_qm"
        }).drop("quantile")
        
        df_agg_qb = df_agg.filter(pl.col("quantile") == self.bins).rename({
                "buy": "buy_qb",
                "sell": "sell_qb",
                "turnover": "turnover_qb",
                "cost": "cost_qb"
        }).drop("quantile")
        df_agg = df_agg_q1.join(df_agg_qm, on=self.trade_date_col, how="left")
        df_agg = df_agg.join(df_agg_qb, on=self.trade_date_col, how="left").with_columns([
            ((pl.col("turnover_q1") + pl.col("turnover_qb")) / 2).alias("turnover_ls"),
            ((pl.col("cost_q1") + pl.col("cost_qb")) / 2).alias("cost_ls"),
            ((pl.col("buy_q1") + pl.col("buy_qb")) / 2).alias("buy_ls"),
            ((pl.col("sell_q1") + pl.col("sell_qb")) / 2).alias("sell_ls")
        ]).sort("datetime")
        ...
        df_q1 = (df_g.filter(pl.col("quantile") == 1).rename({
            "ret": "ret_q1",
            "ret_s": "ret_s_q1",
            "cost": "cost_q1"
            ...
            (pl.col("ret_q1") - pl.col("cost_q1")).alias("ret_net_q1"),
            (pl.col("ret_s_q1") - pl.col("cost_q1")).alias("ret_net_s_q1")
            ])
        
        df_qm = (df_g.filter(pl.col("quantile") == self.mid_q).rename({
            "ret": "ret_qm",
            "ret_s": "ret_s_qm",
            "cost": "cost_qm"
        }).group_by(self.trade_date_col)
            ...
            (pl.col("ret_qm") - pl.col("cost_qm")).alias("ret_net_qm"),
            (pl.col("ret_s_qm") - pl.col("cost_qm")).alias("ret_net_s_qm")
            ])
        
        df_qb = (df_g.filter(pl.col("quantile") == self.bins).rename({
            "ret": "ret_qb",
            "ret_s": "ret_s_qb",
            "cost": "cost_qb"
        }).group_by(self.trade_date_col)
            ...
            (pl.col("ret_qb") - pl.col("cost_qb")).alias("ret_net_qb"),
            (pl.col("ret_s_qb") - pl.col("cost_qb")).alias("ret_net_s_qb")
            ])

        ############################################################
        if self.group_by:
            long_ret_i = df_q1.group_by([self.trade_date_col,'industry']).agg(pl.col("ret_net_q1").mean().alias("ret_net_i")).sort(self.trade_date_col)
            short_ret_i = df_qb.group_by([self.trade_date_col,'industry']).agg(pl.col("ret_net_s_qb").mean().alias("ret_net_s_i")).sort(self.trade_date_col)
            
            df_indus = long_ret_i.join(short_ret_i, on=[self.trade_date_col,'industry'], how="left")
            
            df_indus = df_indus.with_columns((pl.col("ret_net_i") - pl.col("ret_net_s_i")).alias("ret_net_ls_i"))
        
            df_indus = (
                df_indus
                .group_by("industry")
                .agg([
                  pl.col("ret_net_i").mean().alias("ret_net_i_mean"),
                    pl.col("ret_net_s_i").mean().alias("ret_net_s_i_mean"),
                    pl.col("ret_net_ls_i").mean().alias("ret_net_ls_i_mean")
                ]))
        else:
            df_indus = pl.DataFrame()
        #############################################################        
        df_result = df_q1.join(df_qm, on="datetime", how="left")
        if self.group_by:
            df_result = df_result.join(df_qb, on="datetime", how="left").drop("industry")
        df_result = df_result.join(df_qb, on="datetime", how="left").sort(self.trade_date_col)
        
        df_result = df_result.with_columns([
            (1 + pl.col("ret_q1")).cum_prod().alias("cum_ret_q1"),
            (1 + pl.col("ret_net_q1")).cum_prod().alias("cum_ret_net_q1"),
            (1 + pl.col("ret_s_q1")).cum_prod().alias("cum_ret_s_q1"),
            (1 + pl.col("ret_net_s_q1")).cum_prod().alias("cum_ret_net_s_q1"),
            
            (1 + pl.col("ret_qm")).cum_prod().alias("cum_ret_qm"),
            (1 + pl.col("ret_net_qm")).cum_prod().alias("cum_ret_net_qm"),
            (1 + pl.col("ret_s_qm")).cum_prod().alias("cum_ret_s_qm"),
            (1 + pl.col("ret_net_s_qm")).cum_prod().alias("cum_ret_net_s_qm"),
            
            (1 + pl.col("ret_qb")).cum_prod().alias("cum_ret_qb"),
            (1 + pl.col("ret_net_qb")).cum_prod().alias("cum_ret_net_qb"),
            (1 + pl.col("ret_s_qb")).cum_prod().alias("cum_ret_s_qb"),
            (1 + pl.col("ret_net_s_qb")).cum_prod().alias("cum_ret_net_s_qb"),
            
            ((pl.col("ret_q1") - pl.col("ret_qb")) / 2).alias("ret_ls")
        ]).with_columns([(pl.col("ret_ls") - (pl.col("cost_q1") + pl.col("cost_qb")) / 2).alias("ret_net_ls")]).with_columns([
            (1 + pl.col("ret_ls")).cum_prod().alias("cum_ret_ls"),
            (1 + pl.col("ret_net_ls")).cum_prod().alias("cum_ret_net_ls"),
            pl.col("ret_net_ls").rolling_mean(window_size=self.return_rolling_period).alias("ret_net_ls_rolling"),
            pl.col("ret_net_q1").rolling_mean(window_size=self.return_rolling_period).alias("ret_net_q1_rolling"),
            pl.col("ret_net_qm").rolling_mean(window_size=self.return_rolling_period).alias("ret_net_qm_rolling"),
            pl.col("ret_net_qb").rolling_mean(window_size=self.return_rolling_period).alias("ret_net_qb_rolling"),
            pl.col("ret_net_s_q1").rolling_mean(window_size=self.return_rolling_period).alias("ret_net_s_q1_rolling"),
            pl.col("ret_net_s_qm").rolling_mean(window_size=self.return_rolling_period).alias("ret_net_s_qm_rolling"),
            pl.col("ret_net_s_qb").rolling_mean(window_size=self.return_rolling_period).alias("ret_net_s_qb_rolling"),
        ]).drop(["cost_q1","cost_qb"])
        ...
        avg_turnover_l = df_agg['turnover_q1'].drop_nans().mean()
        avg_turnover_s = df_agg['turnover_qb'].drop_nans().mean()
        ...
        if self.agg_freq:
            df_monthly = (
                ...
                .agg([
                    ...
                    ((pl.col("ret_net_q1") + 1).product() - 1).alias("ret_net_q1_agg"),
                    ((pl.col("ret_net_s_qb") + 1).product() - 1).alias("ret_net_s_qb_agg")
                ])
            )
            
            monthly_ret_ls = df_monthly.select(["agg_date", "ret_net_ls_agg"])
            monthly_ret_l = df_monthly.select(["agg_date", "ret_net_q1_agg"])
        ...
        cum_nv = df_result["cum_ret_net_q1"].to_numpy()
        mean_ret_l = df_result.select(pl.col("ret_net_q1").mean()).item()
        mean_loss = (df_result.filter(pl.col("ret_net_q1") < 0).select(pl.col("ret_net_q1").mean()).item())
        PL = mean_ret_l / mean_loss
        ann_ret = cum_nv[-1]** (365.25 / self.days) - 1
        vol = (df_result.select(pl.col("ret_net_q1").std(ddof=1)).item()) * np.sqrt(annual_factor)
        if np.isnan(vol):
            vol = (df_result.select(pl.col("ret_net_q1").std(ddof=0)).item()) * np.sqrt(annual_factor)
        ...
        win_rate = float(df_result.select((pl.col("ret_net_q1") > 0).mean()).item())
        pnl = float(cum_nv[-1] - 1) if cum_nv.size > 0 else np.nan
        downside_std = (df_result.filter(pl.col("ret_net_q1") < 0).select(pl.col("ret_net_q1").std(ddof=1)).item())
        if downside_std is not None and not np.isnan(downside_std):
            downside_vol = downside_std * np.sqrt(annual_factor)
            sortino = excess_ret / downside_vol if downside_vol != 0 else np.nan
        else:
        ...
        ######################################################s
        cum_nv = df_result["cum_ret_net_s_qb"].to_numpy()
        mean_ret_s = df_result.select(pl.col("ret_net_s_qb").mean()).item()
        mean_loss = (df_result.filter(pl.col("ret_net_s_qb") < 0).select(pl.col("ret_net_s_qb").mean()).item())
        PL = mean_ret_s / mean_loss
        ann_ret = cum_nv[-1]** (365.25 / self.days) - 1
        vol = (df_result.select(pl.col("ret_net_s_qb").std(ddof=1)).item()) * np.sqrt(annual_factor)
        if np.isnan(vol):
            vol = (df_result.select(pl.col("ret_net_s_qb").std(ddof=0)).item()) * np.sqrt(annual_factor)
        ...
        win_rate = float(df_result.select((pl.col("ret_net_s_qb") > 0).mean()).item())
        pnl = float(cum_nv[-1] - 1) if cum_nv.size > 0 else np.nan
        downside_std = (df_result.filter(pl.col("ret_net_s_qb") < 0).select(pl.col("ret_net_s_qb").std(ddof=1)).item())
        if downside_std is not None and not np.isnan(downside_std):
            downside_vol = downside_std * np.sqrt(annual_factor)
            sortino = excess_ret / downside_vol if downside_vol != 0 else np.nan
        else:
        ...
        if self.log_scale:
            df_result = df_result.with_columns([
                pl.col("cum_ret_net_q1").log().alias("cum_ret_net_q1"),
                pl.col("cum_ret_net_s_q1").log().alias("cum_ret_net_s_q1"),
                pl.col("cum_ret_net_qm").log().alias("cum_ret_net_qm"),
                pl.col("cum_ret_net_s_qm").log().alias("cum_ret_net_s_qm"),
                pl.col("cum_ret_net_qb").log().alias("cum_ret_net_qb"),
                pl.col("cum_ret_net_s_qb").log().alias("cum_ret_net_s_qb")
            ])
        
        return period, stats_ls, df_result, mean_ret, monthly_ret_ls, monthly_ret_l, stats_l, monthly_ret_s, stats_s, df_indus, avg_turnover_ls, df_agg
    ...
    def calc_stats_for_trace(self,period,date):
        ...
        rename_map = {
            f"ret_q{self.mid_q}": "ret_qm",
            f"ret_q{self.bins}": "ret_qb",

            f"ret_s_q{self.mid_q}": "ret_s_qm",
            f"ret_s_q{self.bins}": "ret_s_qb",

            f"ret_net_q{self.mid_q}": "ret_net_qm",
            f"ret_net_q{self.bins}": "ret_net_qb",

            f"ret_net_s_q{self.mid_q}": "ret_net_s_qm",
            f"ret_net_s_q{self.bins}": "ret_net_s_qb",
        }

        df_result = df_result.rename(rename_map)
        
        df_result = df_result.with_columns([
            ((pl.col("ret_q1") - pl.col("ret_qb")) / 2).alias("ret_ls"),
            ((pl.col("ret_net_q1") - pl.col("ret_net_qb")) / 2).alias("ret_net_ls")
        ])
    ...
    def run_stats_parallel(self):
        ...
        stats_df_ls = pd.DataFrame([{"period": p, **s} for p, s, ls, mean_ret, monthly, monthly_ret_l, stats_l, monthly_s, stats_s, ls_ret_i, avg_turnover_ls, df_agg in results])
        stats_df_l = pd.DataFrame([{"period": p, **stats_l} for p, s, ls, mean_ret, monthly, monthly_ret_l, stats_l, monthly_s, stats_s, ls_ret_i, avg_turnover_ls, df_agg in results])
        stats_df_s = pd.DataFrame([{"period": p, **stats_s} for p, s, ls, mean_ret, monthly, monthly_ret_l, stats_l, monthly_s, stats_s, ls_ret_i, avg_turnover_ls, df_agg in results])
        ls_rets = {p: ls.to_pandas() for p, s, ls, mean_ret, monthly, monthly_ret_l, stats_l, monthly_s, stats_s, ls_ret_i, avg_turnover_ls, df_agg in results}
        ls_turnovers = {p: avg_turnover_ls for p, s, ls, mean_ret, monthly, monthly_ret_l, stats_l, monthly_s, stats_s, ls_ret_i, avg_turnover_ls, df_agg in results}
        mean_rets = {p: mean_ret for p, s, ls, mean_ret, monthly, monthly_ret_l, stats_l, monthly_s, stats_s, ls_ret_i, avg_turnover_ls, df_agg in results}
        indus_rets = {p: ls_ret_i.to_pandas() for p, s, ls, mean_ret, monthly, monthly_ret_l, stats_l, monthly_s, stats_s, ls_ret_i, avg_turnover_ls, df_agg in results}
        agg_dfs = {p: df_agg.to_pandas() for p, s, ls, mean_ret, monthly, monthly_ret_l, stats_l, monthly_s, stats_s, ls_ret_i, avg_turnover_ls, df_agg in results}

        
        monthly_ls = pl.concat(
        [m.with_columns(pl.lit(p).alias("period")) for p, _, _, _, m, monthly_ret_l, stats_l, monthly_s, stats_s, ls_ret_i, avg_turnover_ls, df_agg in results if m.height > 0]
    )
        monthly_l = pl.concat(
        [monthly_ret_l.with_columns(pl.lit(p).alias("period")) for p, _, _, _, m, monthly_ret_l, stats_l, monthly_s, stats_s, ls_ret_i, avg_turnover_ls, df_agg in results if monthly_ret_l.height > 0]
    )
        monthly_s = pl.concat(
        [monthly_s.with_columns(pl.lit(p).alias("period")) for p, _, _, _, m, monthly_ret_l, stats_l, monthly_s, stats_s, ls_ret_i, avg_turnover_ls, df_agg in results if monthly_s.height > 0]
    )
        monthly_pivot_ls = (
        monthly_ls
        .to_pandas()
        ...
        monthly_l
        .to_pandas()
        .pivot(index="agg_date", columns="period", values="ret_net_q1_agg")
        .sort_index()
    )
        monthly_pivot_s = (
        monthly_s
        ...
        return ls_rets, indus_rets, ls_turnovers, ic_dfs, stats_df_ls, stats_df_l, stats_df_s, ic_panal, monthly_pivot_ls.T, monthly_pivot_l.T, monthly_pivot_s.T, monthly_pivot_ic.T, mean_rets, mean_ics, mean_ic_autocorrs, ic_indus_contribs, agg_dfs
    ...
    def create_long_return_sheet(self, staticPlot:bool=False, return_fig:bool=False):
        """
        Generate a comprehensive multi-panel visualization for long-only factor performance analysis.

        This function constructs a vertically stacked Plotly figure containing multiple analytical panels
        based on different rebalance periods. It visualizes rolling returns, cumulative returns, turnover,
        optional industry-level performance, and summary heatmaps.

        Parameters
        ----------
        staticPlot : bool, default False
            If True, renders the figure in static mode (non-interactive). Useful for exporting or reporting.
            If False, enables full interactivity (zoom, hover, etc.).

        return_fig : bool, default False
            If True, returns the Plotly Figure object instead of (or in addition to) displaying it.
            Useful for testing, further customization, or external saving.

        Panels Included
        ---------------
        1. Rolling Mean Returns
            - Displays rolling average returns for top (Q1), middle (Qm), and bottom (Qb) quantiles.
            - Helps identify short-term signal stability.

        2. Cumulative Returns
            - Shows cumulative net value curves for different quantiles.
            - Supports optional log scaling (controlled by self.log_scale).

        3. Industry Analysis (Optional)
            - Only included if self.group_by is True.
            - Horizontal bar chart of average returns by industry for top quantile.

        4. Turnover Rate
            - Scatter plot of turnover ratios for top and bottom quantiles.
            - Indicates trading frequency and strategy cost implications.

        5. Monthly Aggregated Return Heatmap
            - Heatmap of monthly returns for top quantile across rebalance periods.
            - Useful for seasonality and consistency analysis.

        6. Statistics Heatmap
            - Summary metrics (e.g., Sharpe, mean return) across rebalance periods.
            - Provides quick comparison of performance quality.

        Layout Design
        -------------
        - Dynamic number of rows based on:
            * number of rebalance periods
            * whether industry grouping is enabled
        - Custom vertical spacing using manual domain control for precise layout.
        - Independent legends per subplot row for clarity.
        - Right-side aligned legends and colorbars to avoid overlap.
        ...
        for period in self.rebalance_periods:
            ...
            fig.add_trace(
                go.Scatter(
                    x=date,
                    y=self.retruns_dict[period]['ret_net_q1_rolling'],
                    mode="lines",
            ...
            fig.add_trace(
                go.Scatter(
                    x=date,
                    y=self.retruns_dict[period]['ret_net_qm_rolling'],
                    mode="lines",
            ...
            fig.add_trace(
                go.Scatter(
                    x=date,
                    y=self.retruns_dict[period]['ret_net_qb_rolling'],
                    mode="lines",
        ...
        for period in self.rebalance_periods:
            ...
            fig.add_trace(
                go.Scatter(
                    x=date,
                    y=self.retruns_dict[period]['cum_ret_q1'],
                    mode="lines",
                    name="Q-Top CR Before Cost",
                    line=dict(width=line_width, color=colors["qtop"], dash="dot"),
                    showlegend=True,
                ),
                row=row, col=1
            )
            fig.add_trace(
                go.Scatter(
                    x=date,
                    y=self.retruns_dict[period]['cum_ret_net_q1'],
                    mode="lines",
            ...
            fig.add_trace(
                go.Scatter(
                    x=date,
                    y=self.retruns_dict[period]['cum_ret_net_qm'],
                    mode="lines",
            ...
            fig.add_trace(
                go.Scatter(
                    x=date,
                    y=self.retruns_dict[period]['cum_ret_net_qb'],
                    mode="lines",
        ...
        if self.group_by:
            for i ,period in enumerate(self.rebalance_periods):
                ...
                df:pd.DataFrame = self.indus_returns_dict[period][['industry',"ret_net_i_mean"]].sort_values(by='ret_net_i_mean', ascending=True)
        ...
        for period in self.rebalance_periods:
            ...
            fig.add_trace(
                go.Scatter(
                    x=date,
                    y=self.agg_dfs_dict[period]['turnover_q1'],
                    mode="markers",
                    name="Q-Top Turnover",
                    marker=dict(size=6),
            ...
            fig.add_trace(
                go.Scatter(
                    x=date,
                    y=self.agg_dfs_dict[period]['turnover_qb'],
                    mode="markers",
    ...
    def create_long_short_return_sheet(self, staticPlot:bool=False, return_fig:bool=False):
        """
        Generate a comprehensive multi-panel visualization for long-short factor performance analysis.

        This function builds a vertically stacked Plotly figure that summarizes the performance of a
        long-short strategy across multiple rebalance periods. It includes return dynamics, cumulative
        performance, turnover, optional industry attribution, and summary heatmaps.

        Parameters
        ----------
        staticPlot : bool, default False
            If True, renders the figure in static (non-interactive) mode.
            If False, enables full interactivity (zoom, hover, etc.).

        return_fig : bool, default False
            If True, returns the Plotly Figure object for further processing or testing.
            Otherwise, only displays the figure.

        Panels Included
        ---------------
        1. Long-Short Returns
            - Time series of long-short returns.
            - Includes rolling mean and horizontal mean return line.
            - Displays mean value annotation for quick reference.

        2. Cumulative Long-Short Returns
            - Net value curve of the long-short portfolio.
            - Supports log scale (controlled by self.log_scale).

        3. Industry Analysis (Optional)
            - Only shown if self.group_by is True.
            - Horizontal bar chart of average industry-level long-short returns.
            - Helps identify sector contribution to alpha.

        4. Turnover Rate
            - Scatter plot of portfolio turnover over time.
            - Includes mean turnover line and annotation.
            - Useful for understanding trading intensity and cost.

        5. Monthly Aggregated Return Heatmap
            - Heatmap of monthly long-short returns across rebalance periods.
            - Useful for identifying consistency and seasonality.

        6. Statistics Heatmap
            - Summary performance metrics (e.g., Sharpe ratio, mean return).
            - Enables quick comparison across rebalance periods.

        Layout Design
        -------------
        - Dynamic row count based on:
            * number of rebalance periods
            * whether industry grouping is enabled
        - Custom vertical spacing via manual domain control for precise layout.
        - Independent legends for each subplot row to improve readability.
        - Right-aligned legends and colorbars to avoid overlap with plots.
        ...
        for period in self.retruns_dict:
            ...
            fig.add_trace(
                go.Scatter(
                    x=date,
                    y=self.retruns_dict[period]['ret_net_ls_rolling'],
                    mode="lines",
        ...
        for period in self.rebalance_periods:
            ...
            fig.add_trace(
                go.Scatter(
                    x=date,
                    y=self.retruns_dict[period]['cum_ret_ls'],
                    mode="lines",
            ...
            fig.add_trace(
                go.Scatter(
                    x=date,
                    y=self.retruns_dict[period]['cum_ret_net_ls'],
                    mode="lines",
    ...
    def create_single_fac_ic_sheet(self, staticPlot:bool=False, return_fig:bool=False):
        """
        Generate a comprehensive multi-panel visualization for factor IC (Information Coefficient) analysis.

        This function constructs a vertically stacked Plotly figure to evaluate the predictive power of a factor
        across multiple return horizons. It supports both standard IC and Rank IC, and provides time-series,
        ...
        ---------------
        1. IC Time Series
            - Displays IC (or Rank IC) over time.
            - Includes rolling mean and horizontal mean IC line.
            - Annotates average IC value.

        2. Cumulative IC
            - Cumulative sum of IC values over time.
            - Helps assess long-term predictive consistency.

        3. Industry Contribution (Optional)
            - Only included if self.group_by is True.
            - Horizontal bar chart of IC contributions by industry.
            - Identifies sector-level alpha sources.

        4. IC Self-Correlation
            - Measures autocorrelation of IC over time.
            - Indicates signal persistence.
            - Includes mean autocorrelation line and annotation.

        5. Q-Q Plot
            - Compares empirical IC distribution to theoretical normal distribution.
            - Used to assess normality and tail behavior.

        6. Monthly Aggregated IC Heatmap
            - Heatmap of IC aggregated by month and horizon.
            - Useful for stability and seasonality analysis.

        7. IC Statistics Heatmap
            - Summary metrics (e.g., mean IC, ICIR) across horizons.
            - Enables quick comparison of predictive strength.

        Layout Design
        -------------
        - Dynamic number of rows based on:
            * number of return horizons
            * whether industry grouping is enabled
        - Manual domain allocation for precise vertical spacing.
        - Independent legends per subplot row for clarity.
        - Right-aligned legends and colorbars to avoid overlap.
    ...
    def create_short_return_sheet(self, staticPlot:bool=False, return_fig:bool=False):
        """
        Generate a comprehensive multi-panel visualization for short-only factor performance analysis.

        This function constructs a vertically stacked Plotly figure to evaluate the performance of the
        short leg (typically bottom quantile) of a factor strategy across multiple rebalance periods.
        It includes rolling returns, cumulative returns, turnover, optional industry attribution, and
        summary heatmaps.
        ...
        Panels Included
        ---------------
        1. Rolling Short Returns
            - Displays rolling mean returns for different quantiles under short perspective.
            - Typically reflects inverted interpretation (bottom quantile as alpha source).

        2. Cumulative Short Returns
            - Net value curves for short portfolios across quantiles.
            - Helps evaluate profitability and drawdowns of short strategies.
            - Supports optional log scaling (controlled by self.log_scale).

        3. Industry Analysis (Optional)
            - Only included if self.group_by is True.
            - Horizontal bar chart of average industry-level short returns.
            - Identifies sector contributions to short alpha.

        4. Turnover Rate
            - Scatter plot of turnover ratios for short portfolios.
            - Indicates trading frequency and potential transaction cost impact.

        5. Monthly Aggregated Return Heatmap
            - Heatmap of monthly short returns across rebalance periods.
            - Useful for detecting seasonality and consistency.

        6. Statistics Heatmap
            - Summary metrics (e.g., mean return, Sharpe ratio) for short strategies.
            - Enables comparison across rebalance periods.
        ...
        for period in self.rebalance_periods:
            ...
            fig.add_trace(
                go.Scatter(
                    x=date,
                    y=self.retruns_dict[period]['ret_net_s_q1_rolling'],
                    mode="lines",
                    name=f"Q-Bottom Rolling {self.return_rolling_period} Mean",
                    line=dict(width=line_width, color=colors["qbot"]),
                    showlegend=True,
            ...
            fig.add_trace(
                go.Scatter(
                    x=date,
                    y=self.retruns_dict[period]['ret_net_s_qm_rolling'],
                    mode="lines",
                    name=f"Q-{self.mid_q} Rolling {self.return_rolling_period} Mean",
                    line=dict(width=line_width, color=colors["qmid"]),
                    showlegend=True,
            ...
            fig.add_trace(
                go.Scatter(
                    x=date,
                    y=self.retruns_dict[period]['ret_net_s_qb_rolling'],
                    mode="lines",
                    name=f"Q-Top Rolling {self.return_rolling_period} Mean",
                    line=dict(width=line_width, color=colors["qtop"]),
                    showlegend=True
        ...
        for period in self.rebalance_periods:
            ...
            fig.add_trace(
                go.Scatter(
                    x=date,
                    y=self.retruns_dict[period]['cum_ret_s_qb'],
                    mode="lines",
                    name="Q-Bottom CR Before Cost",
                    line=dict(width=line_width, color=colors["qbot"], dash="dot"),
                    showlegend=True,
                ),
                row=row, col=1
            )
            fig.add_trace(
                go.Scatter(
                    x=date,
                    y=self.retruns_dict[period]['cum_ret_net_s_qb'],
                    mode="lines",
                    name="Q-Bottom Cumulative Return",
                    line=dict(width=line_width, color=colors["qbot"]),
                    showlegend=True,
            ...
            fig.add_trace(
                go.Scatter(
                    x=date,
                    y=self.retruns_dict[period]['cum_ret_net_s_qm'],
                    mode="lines",
                    name=f"Q-{self.mid_q} Cumulative Return",
                    line=dict(width=line_width, color=colors["qmid"]),
                    showlegend=True,
            ...
            fig.add_trace(
                go.Scatter(
                    x=date,
                    y=self.retruns_dict[period]['cum_ret_net_s_q1'],
                    mode="lines",
                    name="Q-Top Cumulative Return",
                    line=dict(width=line_width, color=colors["qtop"]),
                    showlegend=True,
        ...
        if self.group_by:
            for i ,period in enumerate(self.rebalance_periods):
                ...
                fig.add_trace(
                    go.Bar(
                        x=df["ret_net_s_i_mean"],
                        y=df["industry"],
                        orientation='h',
                        marker=dict(
                            color=df["ret_net_s_i_mean"],
                            colorscale="RdYlGn",
                    showscale=True, 
                    colorbar=dict(
                        title="Value",       
    ...
    def trace(self, rebalence_period:int|str, date:str, bins:list=None, position="l", staticPlot:bool=False, return_fig:bool=False):
        df_ori, df_result, Dates, rebalence_dates = self.calc_stats_for_trace(rebalence_period,date)
        bins = bins if bins else [1,self.mid_q,self.bins]
        date_map = dict(zip(sorted(Dates)[1:], sorted(Dates)[:-1]))
        
        Dates = set(sorted(Dates)[1:])
        rebalence_dates = set(rebalence_dates)
        rebalence_bars_len = len(Dates & rebalence_dates)
        ###############################################################
        if position == "l":
            cols = ["ret_net_q1", "ret_net_qm", "ret_net_qb"]
        elif position == "s":
            cols = ["ret_net_s_q1", "ret_net_s_qm", "ret_net_s_qb"]
        ...
        try:
            ...
            order_map = {"q1": 0, "qm": 1, "qb": 2}
            label_map = {"q1": "Q-Top", "qm": f"Q-{self.mid_q}", "qb": "Q-Bottom"}

            df_long = (
                df_long.with_columns([
                    pl.col("quantile").replace(order_map).alias("q_order"),
                    pl.col("quantile").replace(label_map).alias("quantile_label")
                ])
                .sort(["q_order", self.trade_date_col])
            )
            ...
            pivot = pivot.select(
                ["quantile_label"] + sorted(pivot.columns[1:])
            )
            z = pivot.select(pl.exclude("quantile_label")).to_numpy()
            y = pivot["quantile_label"].to_list()
            x = pivot.columns[1:]
            self.add_subtitle(fig,'Panel of Returns',row,y=1.1)
            fig.add_trace(
                go.Heatmap(
                    z=z,
                    x=x,
                    ...
                    text=z,
                    texttemplate="%{text:.2%}",
                    hovertemplate="Metric: %{x}<br>Period: %{y}<br>Value:  %{z:.2%}<extra></extra>"
                ),
                row=row, col=1
            )
