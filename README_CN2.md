...
### - 1:1 實盤級回測 — 全向量化模擬交易手續費、滑點、調倉及動態權重漂移，確保回測結果貼合實盤交易。
### - 全透明白盒回測 — 可直接提供交互式報告，觀測任意時間截面、任意持倉方向、任意分箱的因子投資組合的權重，收益率和調倉明細。
...
| 緩存溢出防護 | 🟢 高 (零拷貝) | ❌ 低 (重 DataFrame 緩存) | 🟢 高 | 🟢 高 | 無回測 | 無回測 |
| 截面快照 | ✅ 原生內置 | ❌ 無 | ❌ 無 | ❌ 無 | ❌ 無 | ❌ 無 |
| 配置複雜度 | 🟢 低 | 🔴 高 | 🟡 中等 | 🟢 低 | 🟢 低 | 🟢 低 |
...
### 4. 截面快照：直接提供持倉最細顆粒度報告！可以在任意時間截面、任意持倉方向、任意分箱，觀測因子投資組合的權重，收益率和調倉明細！
```bash
#舉個例子，假設您想了解 2012 年 9 月 28 日當天，季度再平衡的多頭頭寸在第一個分箱中的情況
