...
### - 1:1 实盘级回测 — 全向量化模拟交易手续费、滑点、调仓及动态权重漂移，确保回测结果贴合实盘交易。

### - 全透明白盒回测 — 可直接提供交互式报告，观测任意时间截面、任意持仓方向、任意分箱的因子投资组合的权重，收益率和调仓明细。
...
|:------------------|:------------|:--------|:------------|:------------|:-------------|:-------------|
| 计算速度 | 🚀 极速 (Rust向量化 + 多进程) | ❌ 慢 (重基础设施) | ⚠️ 中等 | ✅ 快 | 无回测 | 无回测 |
| 因子预处理 (40+) | ✅ 原生内置 | ⚠️ 有限制 | ❌ 无 | ❌ 无 | ❌ 无 | ❌ 无 |
| 缓存溢出防护 | 🟢 高 (零拷贝) | ❌ 低 (重 DataFrame 缓存) | 🟢 高 | 🟢 高 | 无回测 | 无回测 |
| 截面快照 | ✅ 原生内置 | ❌ 无 | ❌ 无 | ❌ 无 | ❌ 无 | ❌ 无 |
| 配置复杂度 | 🟢 低 | 🔴 高 | 🟡 中等 | 🟢 低 | 🟢 低 | 🟢 低 |
| IC 分析 | ✅ 原生内置 | ✅ 支持 | ❌ 无 | ✅ 支持 | ❌ 无 | ❌ 无 |
| 多头 / 空头 / 多空再平衡分层回测 | ✅ 原生内置 | ✅ 支持 | ⚠️ 不直接支持 | ❌ 无 | ❌ 无 | ❌ 无 |
...
### 4. 截面快照：直接提供持仓最细颗粒度报告！可以在任意时间截面、任意持仓方向、任意分箱，观测因子投资组合的权重，收益率和调仓明细！

```bash
#举个例子，假设您想了解 2012 年 9 月 28 日当天，季度再平衡的多头头寸在第一个分箱中的情况
...
## 任何问题或反馈？
如果你有任何疑问、遇到了Bug，或者想提出新功能建议，请随时提交 Issue 或发起讨论 Discussion,非常期待听到你的声音~
